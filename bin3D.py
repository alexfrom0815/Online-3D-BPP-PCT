...
class PackingDiscrete(gym.Env):
    ...
    def step(self, action):
        ...
        if not succeeded:
            ...
            all_counter = []
            all_ratio = []
            all_counter.append(len(self.space.boxes))
            all_ratio.append(self.space.get_ratio())
            info = {'counter': np.sum(all_counter), 'ratio': np.sum(all_ratio),
                    'reward': np.sum(all_ratio) * 10}
